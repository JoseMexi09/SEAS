...
$usuario=new Usuario();
echo $usuario->agregar_usu_moral($datos,$idUsuario);
...
//----------------moviendo el archivo 4 acta constitutiva -------------


//////////////////////////////////////////////////////////////////////////////////
if (!empty($inputt3)) {
//empieza a subir archivo 
if($_FILES['acta_c']['size'] > 0){
...
mkdir($carpetaUsuario,0777,true);
  ...
  $totalArchivos=count($_FILES['acta_c']['name']);

  for ($i=0; $i < $totalArchivos; $i++){

    $nombreArchivo=$_FILES['acta_c']['name'][$i];
    $explode = explode('.',$nombreArchivo);
    $tipoArchivo=array_pop($explode);

    $rutaAlmacenamiento=$_FILES['acta_c']['tmp_name'][$i];
